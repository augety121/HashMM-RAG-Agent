...
import { X, FileText, Activity, MonitorX } from "lucide-react";
...
export function WorkbenchView() {
  ...
  const [cockpitTab, setCockpitTab] = useState<"term" | "git">("term");   // V103.12: 驾驶舱底部 终端/Git图 切换
  const [gitOpened, setGitOpened] = useState(false);                       // Git 图懒挂载
    ...
    <div className="flex flex-1 min-h-0">
      ...
      <div className={`flex flex-col min-h-0 ${cockpit ? "flex-shrink-0" : "flex-1"}`}
        ...
        <div className="flex-1 min-h-0 flex flex-col" style={{ height: sel ? "55%" : undefined }}>
          <FileBrowser selPath={sel?.path} onSelect={preview} onDirChange={onDirChange}
        ...
        {sel && (
          <div className="flex-shrink-0 flex flex-col min-h-0" style={{ height: "45%", borderTop: "1px solid var(--border)" }}>
            <div className="flex items-center gap-1.5 px-3 py-1.5 flex-shrink-0" style={{ borderBottom: "1px solid var(--border)", background: "var(--bg-secondary)" }}>
              ...
              {pvLive && <span className="text-[9.5px] px-1.5 rounded-full flex-shrink-0" style={{ background: "var(--accent)", color: "#fff" }}>已更新</span>}
              {pv?.kind === "text" && (
        ...
        <>
          ...
          <div className="flex-1 min-w-0 flex flex-col">
            ...
            <div className="flex items-center gap-1 px-2 pt-1 flex-shrink-0" style={{ borderTop: "1px solid var(--border)" }}>
              {([["term", "终端"], ["git", "Git 图"]] as const).map(([v, label]) => (
                <button key={v} onClick={() => { setCockpitTab(v); if (v === "git") setGitOpened(true); }}
            ...
            <div className="flex-1 min-h-0 relative">
              <div className="absolute inset-0 flex flex-col" style={{ display: cockpitTab === "git" ? "none" : "flex" }}><TerminalView /></div>
              {gitOpened && <div className="absolute inset-0 flex flex-col" style={{ display: cockpitTab === "git" ? "flex" : "none" }}><GitGraphView /></div>}
            </div>
          </div>
        </>
      )}
