...
# Apply the feature preset (HASHMM_PRESET=basic|recommended|max) right after
# .env is loaded and before any module reads the advanced HASHMM_* flags. This
# lets users unlock the project's existing depth with one switch instead of
# setting ~30 obscure env vars. Defaults to "basic" = historical behavior, and
# never overrides a flag the user set explicitly. Guarded so a bug here can
# never block startup.
try:
    from hashmm.feature_presets import apply_preset as _apply_preset

    _applied_flags = _apply_preset()
    if _applied_flags:
        import logging as _logging

        _logging.getLogger("hashmm.config").info(
            "feature preset '%s' enabled %d advanced flag(s): %s",
            os.environ.get("HASHMM_PRESET", "basic"),
            len(_applied_flags),
            ", ".join(sorted(_applied_flags)),
        )
except Exception:  # pragma: no cover - presets must never break startup
    pass
