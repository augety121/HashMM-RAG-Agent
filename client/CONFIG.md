...
> requirements*.txt 已全部 ASCII 化：pip 在无 BOM 时按**系统编码**读该文件（中文 Windows = GBK），
> 任何非 ASCII 字节都会让本地安装直接炸 `UnicodeDecodeError`。中文说明统一放这里。

**requirements.txt（核心，本地后端初始化装的就是这份）**：fastapi/starlette/uvicorn/pydantic = Web 框架与 API；numpy/faiss-cpu = 向量索引（GPU 环境可换 faiss-gpu）；rank_bm25 = BM25 稀疏检索；jieba = 中文分词；networkx = 知识图谱；httpx/requests/openai = HTTP 与 LLM/视觉统一客户端；pypdf/pdfplumber = 语料入库文档解析；PyJWT = 鉴权。

**requirements-optional.txt（可选重型，缺失优雅降级）**：torch/FlagEmbedding/transformers = GPU 嵌入与重排（注释态，按 CUDA 环境自装）；python-pptx/docx/openpyxl/Pillow = 文档生成；playwright = 设计渲染精确截图；redis = 缓存；beautifulsoup4 = 联网检索网页解析；PyPDF2/pandas = 文档转换。
