...
(async () => {
  ...
  {
    const { UpdateService, compareVersions, isNewer } = require("./update-service");
  ...
  // 5. 托盘服务：菜单模板构建（纯逻辑）
  {
    const { buildTrayMenuTemplate, TrayService } = require("./tray-service");
    const clicks = [];
  ...
  {
    const WS = require("./window-service");
    // 切换
  ...
  {
    const TS = require("./terminal-service");
    // 缓冲环截尾
  ...
  {
    const SS = require("./screenshot-util");
    // 1080p@1x：不触顶
  ...
  {
    const { bootstrapServices } = require("./bootstrap");
    const { ServiceRegistry } = require("./registry");
    const reg = new ServiceRegistry();
    const handlers = {};
